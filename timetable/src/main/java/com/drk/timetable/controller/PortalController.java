...
public class PortalController {
    ...
    public String handleRegistration(@ModelAttribute("user") User user, @RequestParam(value = "branch", required = false) String branch,
        ...
        if ("FACULTY".equalsIgnoreCase(user.getRole())) {
            ...
            if (whitelistedTeacher.isEmpty()) {
                model.addAttribute("errorMsg", "Registration Denied! Your email is not whitelisted.");
                return "portal/register";
            }
            user.setFullName(whitelistedTeacher.get().getName());
        } else if ("STUDENT".equalsIgnoreCase(user.getRole())) {
            user.setBranch(branch); user.setAcademicYear(year); user.setSectionName(sectionName); 
        }
    ...
    public String handleLogin(@RequestParam("email") String email, @RequestParam("password") String password, Model model) {
        ...
        if (loggedInUser.isPresent()) {
            ...
            if ("FACULTY".equalsIgnoreCase(user.getRole())) {
                model.addAttribute("mySchedule", timetableEntryRepository.findByTeacherName(user.getFullName()));
                return "portal/faculty-dashboard";
            } else {
                model.addAttribute("mySchedule", timetableEntryRepository.findByAcademicYearAndBranchAndSectionName(user.getAcademicYear(), user.getBranch(), user.getSectionName()));
    ...
    // --- ADMIN ROUTES (The full original logic) ---
